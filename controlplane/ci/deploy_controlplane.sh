...
GW_TAG="gateway-v0.22.0"
GW_SHA="220a3fb3999646c949c9fec6a2c5eb57d63993646aa7b9adc05712dc3aaaa69b"
...
# SKU/CUDA-class row. This is an experiment substrate selector, not a planner
...
if [ -n "$BOOTSTRAP_ONLY_GPU_SKU" ] || [ -n "$BOOTSTRAP_ONLY_CUDA_CLASS" ]; then
  if [ -z "$BOOTSTRAP_ONLY_GPU_SKU" ] || [ -z "$BOOTSTRAP_ONLY_CUDA_CLASS" ]; then
    echo "WEINFER_BOOTSTRAP_ONLY_GPU_SKU and WEINFER_BOOTSTRAP_ONLY_CUDA_CLASS must be set together" >&2
    exit 1
  fi
  BOOTSTRAP_HARDWARE=$(python3 - "$DEFAULT_BOOTSTRAP_HARDWARE" "$BOOTSTRAP_ONLY_GPU_SKU" "$BOOTSTRAP_ONLY_CUDA_CLASS" <<'PY'
import json, sys
